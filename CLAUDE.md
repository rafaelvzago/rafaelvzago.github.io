# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

**Process (SDLC):** follow [`docs/agents/sdlc.md`](docs/agents/sdlc.md). This file and `AGENTS.md` cover commands and domain; they do not override the playbook.

## Project Overview

This is Rafael Zago's personal blog built with Hugo using the Archie theme. The site is written in Portuguese (pt-BR) and focuses on cloud, DevOps, infrastructure, automation, and AI-related topics. The site is deployed to GitHub Pages.

## Development Commands

### Basic Hugo Operations
- `git submodule update --init --recursive` - Fetch the Archie theme
- `hugo server` / `make serve` - Run development server (usually at http://localhost:1313)
- `hugo --minify` / `make build` - Build the site for production (`public/`)
- `hugo mod` / theme updates via git submodule

### Testing
- `make test` - **MUST pass before any task is considered complete**. Runs `hugo --minify` and htmlproofer on `./public` when available (otherwise artifact/tracker smoke checks)

## Content Structure

### Posts
- Posts live in `content/pt-br/posts/` and `content/en/posts/` (`YYYY-MM-DD-slug.md`)
- Matching relative paths link translations (Hugo multilingual)
- Posts must include frontmatter with `title`, `description`, `date`, `slug`, and `tags`
- Default locale is Portuguese (`/posts/<slug>/`); English is under `/en/posts/<slug>/`

### Static Pages
- About lives at `content/about.md`
- Tags archive is generated by Hugo/Archie at `/tags/`

### Assets
- Images are stored in `static/assets/` (URL path `/assets/...`)
- Header images for posts are in `static/assets/img/headers/`
- Favicons are in `static/assets/img/favicons/`

### Sub-Sites
- `static/qr/` - QR code redirect system (spec: `specs/qr-system/spec.md`). Accessed via direct URL only
- `static/amigo/` - Secret Santa (Amigo Secreto) reveal app. Standalone HTML. Accessed via direct URL only
- Sub-sites are NOT linked from the homepage navigation

### QR Code System
- Published files: `static/qr/`
- Generator tooling: `scripts/qr/gerar_qrcodes.py` + `scripts/qr/urls.txt`
- QR codes redirect to the site with ID parameters for tracking

## Key Configuration

- **Theme**: Archie (`themes/archie` git submodule)
- **Language**: Portuguese (pt-BR)
- **Timezone**: America/Sao_Paulo
- **Base URL**: https://www.rafaelvzago.com
- **Google Analytics**: G-9J3YRPN8EN
- **Google Tag Manager**: GTM-592PS4S2
- **Config file**: `hugo.toml`

### Dependency Management
- Pin Hugo Extended version in CI (`.github/workflows/pages-deploy.yml`)
- Keep the Archie theme submodule in sync; verify with `make test`

## Git Workflow

- **Main branch**: `main` (for production/GitHub Pages)
- **Development branch**: `development` (for active development)
- Lastmod uses Hugo `enableGitInfo` (CI must checkout with `fetch-depth: 0`)

## Content Guidelines

- Posts should focus on cloud, DevOps, infrastructure, automation, Linux, containers, Kubernetes, AI/ML topics
- All content is in Portuguese
- Use appropriate header images from `static/assets/img/headers/`
- Include relevant tags in post frontmatter
- TOC via `toc: true` in frontmatter

## Content Quality

All user-facing content (blog posts, About page text) MUST be processed through the `/humanizer` skill before being considered final. This includes:
- New blog posts
- Significant edits to existing posts
- About page or other page text updates

Run `/humanizer` on the content before committing. This does NOT apply to technical documentation (CLAUDE.md, specs, README) or configuration files.

## Spec-Driven Development (SDD)

This project uses [GitHub spec-kit](https://github.com/github/spec-kit) for Spec-Driven Development. Specs are structured, behavior-oriented documents that serve as the source of truth for AI-assisted changes.

### Directory Layout

- `.specify/memory/constitution.md` - Project principles (read this before any change)
- `.specify/templates/` - Templates for new specs, plans, and tasks
- `.specify/scripts/` - Automation scripts for the SDD workflow
- `.claude/skills/` - Spec-kit slash commands for Claude
- `specs/` - Feature specification directories (one folder per feature)

### Existing Baseline Specs

- `specs/site-core/` - Hugo + Archie configuration, theme, analytics
- `specs/content-workflow/` - Blog post lifecycle, frontmatter, images, gitinfo
- `specs/deployment/` - GitHub Actions CI/CD pipeline, gh-pages, CNAME
- `specs/qr-system/` - QR code generation and redirect system

### Workflow (for new features or significant changes)

1. **Read the constitution** (`.specify/memory/constitution.md`) before starting
2. **Check existing specs** in `specs/` for related features
3. **Create a spec** using `/speckit-specify` for new features
4. **Plan implementation** using `/speckit-plan` with tech stack details
5. **Break into tasks** using `/speckit-tasks`
6. **Implement** using `/speckit-implement`

### When to use SDD

- Adding a new site feature (new page, widget, integration)
- Making structural changes (config, deployment, theme customization)
- Modifying the QR system or adding a new subsystem

### When NOT to use SDD

- Writing or editing a blog post (just follow the content-workflow spec)
- Fixing a typo or broken link
- Updating a dependency version

## Code Style

- **Indentation**: Use 4 spaces for indentation (no tabs)

## Commit Message Standard

### Format

```
<type>(<scope>): <short description>

[optional body]
```

**Rules:**
- All commits MUST follow this format. Non-compliant commits MUST be amended before merging
- Type and scope in English (conventional standard)
- Description in English (keeps history searchable and consistent)
- 72-character limit on the subject line
- Imperative mood: "add post about X", not "added post about X"

### Types

| Type | When to use |
|------|-------------|
| `post` | New blog post published |
| `draft` | New draft post (not yet published) |
| `fix` | Typo corrections, broken links, layout bugs |
| `feat` | New site feature (new page, new section, new widget) |
| `chore` | Dependency updates, Makefile, tooling |
| `config` | Changes to `hugo.toml` or Hugo/theme settings |
| `ci` | GitHub Actions, deployment workflow changes |
| `style` | CSS, layout, or visual-only changes |
| `docs` | Changes to README, CLAUDE.md, or meta-documentation |
| `asset` | New or updated images, favicons, static files |

### Scopes (optional but recommended)

| Scope | Meaning |
|-------|---------|
| `post/<slug>` | Specific post (e.g., `post(post/intro-k8s)`) |
| `about` | About page |
| `theme` | Archie theme customization / overrides |
| `deps` | Gem/bundler dependencies |
| `analytics` | Google Analytics config |
| `qr` | QR code system |

### Examples

```
post(post/intro-openshift): add new post about OpenShift service mesh
fix(post/deepseek): correct typos in first act section
chore(deps): bump Hugo Extended version in CI
config: enable gitinfo lastmod for posts
asset: add header image for llm post
feat: add tag cloud to sidebar
ci: update github pages deployment workflow
docs(claude): add commit message standard to CLAUDE.md
```

## Agent System

This repository uses a Claude Code agent system with specialized agents:

- **Writer** (`/write-post`) -- Creates new blog posts following established patterns
- **Reviewer** (`/review-post`) -- Reviews posts for quality, formatting, and SEO
- **Content Analyst** (`/analyze-content`) -- Analyzes content gaps and suggests topics
- **Committer** (`/commit`) -- Creates conventional commits with pre-commit checks

Agents support subagent chaining (e.g., Writer can auto-spawn Reviewer after creating a post).

See `AGENTS.md` for behavioral rules and invariants.
See `.claude/agents/` for agent definitions, `.claude/skills/` for workflows, `.claude/commands/` for invocation.
See `docs/agents/sdlc.md` for the agentic development playbook.
